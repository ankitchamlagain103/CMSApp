...
position, employment status), and bank payment details. `Teacher` (`/api/teachers`, unchanged
routes) is only teaching-specific data (`teachingLicenseNo`, `experienceYears`, `specialization`)
attached to an Employee via a **shared primary key**: `Teacher.Id` is always equal to its owning
`Employee.Id`. This is what let `TeacherAssignment`/`TeacherDocument`/`TeacherQualification` stay
**completely unchanged** — their `teacherId` FK still means exactly what it meant before, it just
now also happens to be an Employee id.
...
|---|---|---|
| `POST /api/employees` | `{ "employeeCode": null, "firstName", "middleName", "lastName", "gender": 0, "dateOfBirth", "email", "phone", "joinDate", "employeeCategoryCode": "ADMINISTRATION", "jobPositionCode": "RECEPTIONIST", "bankName", "bankAccountNumber", "paymentMode": 1 }` | `employeeCode` optional — blank = auto-generated `EMP{year}{seq}` (one sequence shared across every employee type, including teachers). `employeeCategoryCode`/`jobPositionCode` validated against catalogs `1011`/`1012`. `paymentMode`: `1` BankDeposit / `2` Cash / `3` Cheque |
...
| `GET /api/employees/{id}/salaries/tax-calculation?fiscalYearId=` | | See below |
...
`/api/teachers/{id}/salaries` and `/api/teachers/{id}/salaries/tax-calculation` still work — they're
thin aliases (`TeacherService` forwards to the same `IEmployeeService` methods using the teacher's
own id, which via the shared-PK trick *is* its Employee id).
...
`dbo.teacher_qualifications`/`dbo.teacher_documents`/`dbo.teacher_assignments` are **unchanged**.
...
`teacher_assignments`/`teacher_documents`/`teacher_qualifications` keep resolving with zero data
changes to them. Not created here, per this repo's convention — run `dotnet ef migrations add`
yourself.
